export async function up(knex: any): Promise<void> {
  ...
  select d.name as id, max(d.host),max(d.port),max(d.name), max(d.dialect),JSON_AGG(c.*) as credentials, JSON_AGG(v.name) as vocab_schemas, '[]' as publications, x.value as db_extra from db_credentials_mgr.db as d 
