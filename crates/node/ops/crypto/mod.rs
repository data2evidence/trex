...
#[op2(fast)]
pub fn op_node_check_prime(#[bigint] num: i64, #[number] checks: usize) -> bool {
...
pub async fn op_node_check_prime_async(
    ...
    Ok(spawn_blocking(move || primes::is_probably_prime(&BigInt::from(num), checks)).await?)
}
...
pub fn op_node_check_prime_bytes_async(
    ...
    // TODO(@littledivy): use rayon for CPU-bound tasks
    Ok(
        async move { Ok(spawn_blocking(move || primes::is_probably_prime(&candidate, checks)).await?) },
    )
}

#[op2(fast)]
#[smi]
pub fn op_node_create_hash(state: &mut OpState, #[string] algorithm: &str) -> u32 {
    state
        .resource_table
...
#[op2(fast)]
pub fn op_node_hash_update(state: &mut OpState, #[smi] rid: u32, #[buffer] data: &[u8]) -> bool {
...
#[op2(fast)]
pub fn op_node_hash_update_str(state: &mut OpState, #[smi] rid: u32, #[string] data: &str) -> bool {
    let context = match state.resource_table.get::<digest::Context>(rid) {
        Ok(context) => context,
        _ => return false,
    };
...
pub fn op_node_hash_digest(
    ...
    let context =
        Rc::try_unwrap(context).map_err(|_| type_error("Hash context is already in use"))?;
...
pub fn op_node_cipheriv_final(
    ...
    let context =
        Rc::try_unwrap(context).map_err(|_| type_error("Cipher context is already in use"))?;
...
pub fn op_node_sign(
    ...
    let (label, doc) = pkcs8::SecretDocument::from_pem(std::str::from_utf8(&key).unwrap())?;
...
#[op2]
#[string]
pub fn op_node_export_rsa_public_pem(#[buffer] pkcs1_der: &[u8]) -> Result<String, AnyError> {
...
#[op2]
#[serde]
pub fn op_node_export_rsa_spki_der(#[buffer] pkcs1_der: &[u8]) -> Result<ToJsBuffer, AnyError> {
    let public_key = RsaPublicKey::from_pkcs1_der(pkcs1_der)?;
...
fn ec_generate(named_curve: &str) -> Result<(ToJsBuffer, ToJsBuffer), AnyError> {
    let mut rng = rand::thread_rng();
...
#[op2(async)]
#[serde]
pub async fn op_node_ed25519_generate_async() -> Result<(ToJsBuffer, ToJsBuffer), AnyError> {
...
#[op2(async)]
#[serde]
pub async fn op_node_x25519_generate_async() -> Result<(ToJsBuffer, ToJsBuffer), AnyError> {
    spawn_blocking(x25519_generate).await?
}

fn dh_generate_group(group_name: &str) -> Result<(ToJsBuffer, ToJsBuffer), AnyError> {
...
pub async fn op_node_dh_generate_async(
    ...
    spawn_blocking(move || dh_generate(prime.as_deref(), prime_len, generator)).await?
}

#[op2(fast)]
#[smi]
pub fn op_node_random_int(#[smi] min: i32, #[smi] max: i32) -> Result<i32, AnyError> {
    let mut rng = rand::thread_rng();
...
fn scrypt(
    ...
    // Construct Params
    let params =
        scrypt::Params::new(cost as u8, block_size, parallelization, keylen as usize).unwrap();
...
pub fn op_node_ecdh_encode_pubkey(
    ...
    match curve {
        "secp256k1" => {
            let pubkey = elliptic_curve::PublicKey::<k256::Secp256k1>::from_encoded_point(
                &elliptic_curve::sec1::EncodedPoint::<k256::Secp256k1>::from_bytes(pubkey)?,
            );
...
pub fn op_node_ecdh_generate_keys(
    ...
    match curve {
        "secp256k1" => {
            let privkey = elliptic_curve::SecretKey::<k256::Secp256k1>::random(&mut rng);
    ...
    match curve {
        "secp256k1" => {
            let their_public_key =
                elliptic_curve::PublicKey::<k256::Secp256k1>::from_sec1_bytes(their_pub)
                    .expect("bad public key");
            let this_private_key = elliptic_curve::SecretKey::<k256::Secp256k1>::from_slice(
...
pub fn op_node_ecdh_compute_public_key(
    ...
    match curve {
        ...
        "prime256v1" | "secp256r1" => {
            let this_private_key = elliptic_curve::SecretKey::<NistP256>::from_slice(privkey)
                .expect("bad private key");
            let public_key = this_private_key.public_key();
            pubkey.copy_from_slice(public_key.to_sec1_bytes().as_ref());
            Ok(())
        ...
        "secp384r1" => {
            let this_private_key = elliptic_curve::SecretKey::<NistP384>::from_slice(privkey)
                .expect("bad private key");
            let public_key = this_private_key.public_key();
            pubkey.copy_from_slice(public_key.to_sec1_bytes().as_ref());
            Ok(())
        ...
        "secp224r1" => {
            let this_private_key = elliptic_curve::SecretKey::<NistP224>::from_slice(privkey)
                .expect("bad private key");
            let public_key = this_private_key.public_key();
            pubkey.copy_from_slice(public_key.to_sec1_bytes().as_ref());
            Ok(())
...
#[op2(async)]
#[serde]
pub async fn op_node_gen_prime_async(#[number] size: usize) -> Result<ToJsBuffer, AnyError> {
...
// }
static MGF1_SHA1_MASK_ALGORITHM: Lazy<rsa::pkcs8::AlgorithmIdentifierRef<'static>> =
    Lazy::new(|| rsa::pkcs8::AlgorithmIdentifierRef {
        // id-mgf1
        oid: ID_MFG1,
        // sha1
        parameters: Some(asn1::AnyRef::from_der(&ENCODED_SHA1_HASH_ALGORITHM).unwrap()),
    });
    ...
    fn try_from(
        ...
        any.sequence(|decoder| {
            ...
            let salt_length = decoder
                .context_specific::<u32>(SALT_LENGTH_TAG, pkcs8::der::TagMode::Explicit)?
                .map(TryInto::try_into)
                .transpose()?
                .unwrap_or(20);
...
fn parse_private_key(
    ...
    match format {
        "pem" => {
            let (_, doc) = pkcs8::SecretDocument::from_pem(std::str::from_utf8(key).unwrap())?;
            Ok(doc)
        }
        "der" => {
            match type_ {
...
fn parse_public_key(key: &[u8], format: &str, type_: &str) -> Result<pkcs8::Document, AnyError> {
    match format {
        "pem" => {
            let (label, doc) = pkcs8::Document::from_pem(std::str::from_utf8(key).unwrap())?;
            if label != "PUBLIC KEY" {
