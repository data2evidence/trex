...
pub fn get_startup_handler(auth_type: &AuthType) -> Arc<Md5PasswordAuthStartupHandler<TrexAuthSource,DefaultServerParameterProvider>>  {
    match auth_type {
        AuthType::Default {password}=> {
            Arc::new(Md5PasswordAuthStartupHandler::new(
                Arc::new(TrexAuthSource {password: password.to_string()}),
                Arc::new(DefaultServerParameterProvider::default()),
            ))
        }
        AuthType::Scram {password, key_slice, cert_slice} => {
            Arc::new(Md5PasswordAuthStartupHandler::new(
                Arc::new(TrexAuthSource {password: password.to_string()}),
                Arc::new(DefaultServerParameterProvider::default()),
            ))
